...
initial_fake_date = "2024-10-22 09:03:23"
...
def send_email(container, fake_date, recipient, subject, body, user):
    ...
    # Split the recipients and prepare them as individual addresses
    recipient_list = recipient.split(" ")
    recipient_string = " ".join([r.strip() for r in recipient_list])

    command = f"echo '{body}' | sudo -u {user} faketime '{fake_date}' mutt -s '{subject}' {recipient_string}"
...
def separate_name_email(name_email):
    # Regular expression to match the format 'Name <email>'
...
def zip_ransomware_folder(container):
    print(f"Inside the {container} container: zipping the folder containing the ransomware...")

    working_dir = "/usr/local/littleProject"
    zip_name = "dbUpdate.zip"
    target_folder = "dbUpdate"

    command = f"cd {working_dir} && zip -r {zip_name} {target_folder}"

    subprocess.run(["docker", "exec", container, "bash", "-c", command])
    print(f"{target_folder} zipped!\n")

def unzip_ransomware_folder(container, fake_date, user, target_zip, working_dir):
    print(f"Inside the {container} container with {user}: unzipping the folder containing the ransomware...")
 
    command = f"cd {working_dir} && sudo -u {user} faketime '{fake_date}' unzip {target_zip}"

    subprocess.run(["docker", "exec", container, "bash", "-c", command])
    print(f"{target_zip} unzipped!\n")

def save_attachment_from_email(container, user):
    command = f"sudo -u {user} python3 /usr/local/bin/save_attachments.py"

    subprocess.run(["docker", "exec", container, "bash", "-c", command])

def run_ransomware_script(container, user):
